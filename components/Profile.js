...
import { View, StyleSheet, Text, Image, ActivityIndicator, BackHandler } from 'react-native';
...
export default function Profile({ navigation }) {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        setLoading(false);
        if (response.data.statusCode === 200 && response.data.engProfile) {
  ...
  return (loading == true ? (
    ...
    <View style={styles.container}>
      <View
        ...
        <Image
          style={{ width: 130, height: 140, margin: 10, marginTop:-5, marginBottom:-5 }}
